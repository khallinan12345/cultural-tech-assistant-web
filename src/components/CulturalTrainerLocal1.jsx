...
const CulturalTrainer = () => {
  ...
  const [llamaServerUrl, setLlamaServerUrl] = useState('http://localhost:3001');
  ...
  // Function to make API call to local Llama server
const callLlamaAPI = async (prompt, role) => {
  ...
  try {
    ...
    // Format prompt with system role for Llama models
    ...
    const response = await fetch(`${llamaServerUrl}/api/generate`, {
      ...
      body: JSON.stringify({
        ...
        // Don't include max_tokens or temperature here - server will handle it
      })
    });
  ...
  const generateProgressEvaluation = async () => {
    ...
    try {
      ...
      return `Error generating progress evaluation: ${error.message}. Please check your Llama server and try again.`;
    }
  };
  ...
  // Function to evaluate user's response using Llama
  const evaluateResponse = async (response, lastClientMessage) => {
    ...
    try {
      ...
      return `Error evaluating response: ${error.message}. Please check your Llama server and try again.`;
    }
  };
  
  // Function to generate client response using Llama
  const generateClientResponse = async (userMessage) => {
    ...
    try {
      ...
      return `Error generating response: ${error.message}. Please check your Llama server and try again.`;
    }
  };
    ...
    <div className="app-container">
      ...
      {showServerConfig ? (
        <div className="server-config-container">
          <h2>Llama Server Configuration</h2>
          <p>Please specify the URL of your local Llama 2 server:</p>
  ...
  <ol>
    <li>Install llama.cpp from <a href="https://github.com/ggerganov/llama.cpp" target="_blank" rel="noopener noreferrer">https://github.com/ggerganov/llama.cpp</a></li>
    <li>Download Llama 2 model weights from Meta</li>
    <li>Start the Llama server using: <code>./build/bin/llama-server -m ./models/llama-2-7b-chat.Q4_K_M.gguf -c 2048</code></li>
    <li>The Llama server should be running on http://localhost:8080</li>
    <li>Then start this Node.js server with: <code>node server.js</code></li>
  </ol>
</div>
        </div>
      ) : (
